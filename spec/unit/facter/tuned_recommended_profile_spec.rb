...
describe ':tuned_recommended_profile', type: :fact do
  ...
  context 'tuned recommend disabled' do
    it do
      ...
      allow(Facter::Core::Execution).to receive(:execute).with('tuned-adm recommend', hash_including(on_fail: nil, logger: instance_of(Logger))).and_return('')
  ...
  context 'tuned running' do
    it do
      allow(Facter::Core::Execution).to receive(:which).with('tuned-adm').and_return('tuned-adm')
      allow(Facter::Core::Execution).to receive(:execute).and_call_original
      allow(Facter::Core::Execution).to receive(:execute).with('tuned-adm recommend', hash_including(on_fail: nil, logger: instance_of(Logger))).and_return(recommend_output)
